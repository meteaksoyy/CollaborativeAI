...
# -------------------------
# Choose the TWO agents here
# Example: Random is not one of your own agents, so this script is for your own 4 agents.
# Change these two lines to any pair you want.
# -------------------------
AGENT_A = BaseNegotiator
AGENT_B = ReactiveT4TNegotiator
...
# -------------------------
# Run the two agents on all scenarios
# -------------------------
def run_all_scenarios(agent_a, agent_b, sessions=50):
    scenarios = {
...
if __name__ == "__main__":
    results_ab = run_all_scenarios(AGENT_A, AGENT_B, sessions=50)

    file_ab = os.path.join(
        SAVE_DIR,
        f"{AGENT_A.__name__}_vs_{AGENT_B.__name__}_all_scenarios_50matches.pkl"
    )
    with open(file_ab, "wb") as f:
        pickle.dump(results_ab, f)
    print(f"Saved {file_ab}")

    results_ba = run_all_scenarios(AGENT_B, AGENT_A, sessions=50)

    file_ba = os.path.join(
        SAVE_DIR,
        f"{AGENT_B.__name__}_vs_{AGENT_A.__name__}_all_scenarios_50matches.pkl"
    )
    with open(file_ba, "wb") as f:
        pickle.dump(results_ba, f)
    print(f"Saved {file_ba}")