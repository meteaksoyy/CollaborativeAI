...
class BaseNegotiator(SAONegotiator):
    ...
    def propose(self, state, *args, **kwargs):
        ...
        u_min = self._rv                                          # reservation value
        u_max = max(self.ufun(o) for o in outcomes)              # actual max utility
        ...
        for outcome in outcomes:                                  # all possible combinations of issues
    ...
    def respond(self, state, offer=None, *args, **kwargs):
        offer = offer if offer is not None else state.current_offer
        ...
        outcomes = self._all_outcomes()
        u_min = self._rv
        u_max = max(self.ufun(o) for o in outcomes)
